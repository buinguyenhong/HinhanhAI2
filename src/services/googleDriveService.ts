...
export async function authenticateWithGoogleDrive(): Promise<{ token: string; email: string }> {
  throw new Error('Google Drive OAuth is not configured. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET, then implement the server-side OAuth callback before enabling Drive uploads.');
