  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Mã truy cập không hợp lệ. Vui lòng kiểm tra lại.');
      }
    } catch {
      ...
      <div className="w-full max-w-sm mx-auto my-auto py-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-4">
            ...
            <p className="text-[10px] text-[#9C988F] dark:text-[#3E3D38] tracking-wide text-center">Phiên đăng nhập được bảo vệ bởi cookie HttpOnly.</p>
          </div>
        </form>
      </div>
